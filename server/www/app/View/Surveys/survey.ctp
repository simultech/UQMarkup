<h2>Markup Survey for <?php echo $course['name']; ?></h2>
    ...
    if (isset($existingresponses[$question[0]])) {
      if ($existingresponses[$question[0]] == "yes") {
		...
		case 'likeit_optional':
			...
			$fields .= '<div class="likeitbox"><input type="radio" '.$box6.' name="data[response]['.$question[0].']" value="0" id="data_response_'.$question[0].'_0" /><label for="data_response_'.$question[0].'_0">N/A</label></div>';
			$fields .= '<div style="clear:both;"></div>';
			$fields .= '</div>';
			break;
	}
...
<input type='submit' value='Save Survey' class='btn btn-primary' /><span id="submit_result"></span>
</p>
<p><em>Note: The survey may be saved and resumed at any time.  Please bookmark <a href="<?php echo $_SERVER['REQUEST_URI']; ?>">this URL</a> to return to the survey later.</em></p>
</form>
...
<style type='text/css'>
	...
	div.likeitspacer {
		margin-left:180px;
		background:url('<?php echo $baseURL ?>/img/likeitline.png') no-repeat 55px 10px;
	}
